...
/**
 * States: `static` (full tiling at rest) → `drift` (slowly drifts and breathes) → `faded` (dimmed to
 * FADED_OPACITY; keeps drifting if it was) → `open` (gaps widen to the open logo's, in step with it).
 */
 ...
 * opacities) in monochrome, aligned so that the hexagon sits at `origin`. Re-tiles on resize.
 */
export const Mesh = (props: MeshProps) => {
  const [local, rest] = splitProps(props, ['class', 'origin', 'unit', 'hole', 'controller', 'onDrift']);
    ...
    const set = async (state: MeshState) => {
      ...
      try {
        switch (state) {
          case 'static':
            ...
            await Promise.all([
              drifter
                ...
                .end(),
              plane.transition().duration(SETTLE_MS).attr('opacity', 1).end(),
              tiles()
                .transition()
                .duration(SETTLE_MS)
          ...
          case 'drift':
            drift(0, 1);
            await plane.transition().duration(SETTLE_MS).attr('opacity', 1).end();
            break;
          case 'faded':
